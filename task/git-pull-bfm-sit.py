...
target_branch = "feature/bfm-batch-2"

def run():
    """Perform these steps:

    - Ensure repo at ~/work/bfm-backend exists
    - Fetch and update local `{target_branch}` branch from `origin/{target_branch}`
    ...
    - Run `scp-bfm-sit <path>` in an interactive zsh so any shell function/alias is available
    ...
    logger.info("Running task: git-pull-bfm-sit")
    repo_dir = os.path.expanduser("~/dev/CMB/bfm-backend")
    ...
    try:
        ...
        run_cmd(f"{shlex.quote(build_script)} backend", cwd=repo_dir)
        ...
        pattern = os.path.join(repo_dir, "**", "bfm-backend-*.tar.gz")
        ...
        cmd = f"scp-bfm-sit {shlex.quote(latest)}"
